...
/*
	tree_node(string s, int v, struct tree_node * l, struct tree_node * r){
		symbols = s;
		value = v;
		left_child = l;
		right_child = r;
...
//
tree_node makenode(string s, int v, struct tree_node * l, struct tree_node * r){
...
int main(int argc, char *argv[]){
	...
	//struct tree_node insertnode; //("l", 0, NULL, NULL);
	string jank = "";
	int curr_head = 0;
	...
	for(auto it = symbol_map.begin(); it != symbol_map.end(); it++){
		...
		jank += it->first;
		cout << "jank " << jank << endl;
		// insertnode = new tree_node; //{jank, it->second.frequency, NULL, NULL};
		insertnode->symbols = jank;
		insertnode->value = it->second.frequency;
		insertnode->left_child = NULL;
		insertnode->right_child = NULL;
		sorted_symbols.push_back(insertnode);
		cout << sorted_symbols[sorted_symbols.size()-1]->symbols << endl;
		curr_head++;
		jank = "";
	}
...
*/
		...
		cout  << sorted_symbols[sorted_symbols.size()-2]->symbols << " is the left child and " << sorted_symbols[sorted_symbols.size()-1]->symbols << " is the right child\n"; 
...
void BFS(tree_node &tree, vector<string> &array){
	...
	while(true){
		...
		if(dash_count == 4) break;
		struct tree_node curr_head = unvisitedFringe.front();
		if(curr_head.left_child == NULL && curr_head.right_child == NULL){
			if(curr_head.symbols == unvisitedFringe.back().symbols) break; //cheating! doing .symbols instead of making a =
			array.push_back("-");
			//number_of_inserts++;
			array.push_back("-");
			dash_count += 2;
			...
			// cout << "hiere" <<  (curr_head.left_child->symbols) << endl;
			unvisitedFringe.push(*(curr_head.right_child));
			array.push_back(curr_head.right_child->symbols);
			// cout << "heerere" << (curr_head.right_child->symbols) << endl;
			dash_count = 0;
		}
		unvisitedFringe.pop();
	}
